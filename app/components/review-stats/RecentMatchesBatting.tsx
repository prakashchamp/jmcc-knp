...
export function RecentMatchesBatting({ matches, performances, loading }: RecentMatchesBattingProps) {
  ...
  const sortedPerformances = useMemo(() => {
    const sorted = [...relevantPerformances].sort((a, b) => {
      let aVal: string | number = '';
      let bVal: string | number = '';
      ...
      if (sortField === 'playerName') {
        ...
        aVal = matchA?.opponent || '';
        bVal = matchB?.opponent || '';
      } else if (sortField === 'runs') {
        aVal = a.batting.runs;
        bVal = b.batting.runs;
      } else if (sortField === 'balls') {
    ...
    return sorted;
  }, [relevantPerformances, matches, sortField, sortDirection]);
            ...
            {sortedPerformances.map((perf, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-center text-gray-300">{match ? formatDate(match.date) : '-'}</td>
                  <td className="px-4 py-3 text-center text-gray-300 text-xs">{match ? match.opponent : '-'}</td>
                  <td className="px-4 py-3 text-center font-semibold text-blue-400">{perf.batting.runs}</td>
                  ...
                  <td className="px-4 py-3 text-center text-orange-400">
                    {perf.batting.strikeRate > 0 ? perf.batting.strikeRate.toFixed(1) : '-'}
                  </td>
                </tr>
              );
            })}
