...
export function useMonthlyStats(month?: string) {
  ...
  useEffect(() => {
    try {
      ...
      // Filter performances by month if specified
      const filterByMonth = month ? (perf: Performance) => perf.month === month : () => true;
      const filteredPerformances = MOCK_PERFORMANCES.filter(filterByMonth);
